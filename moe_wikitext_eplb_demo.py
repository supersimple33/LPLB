"""Tiny MoE language-model demo with EPLB (Elastic Placement Load Balancing).

Run with 4 GPUs, for example:

    torchrun --nproc_per_node=4 moe_wikitext_eplb_demo.py --steps 200
...
This demo uses EPLB to dynamically move non-redundant experts across GPUs based
on current workload, with no redundancy.
"""
...
class EPLBMoE(nn.Module):
    def __init__(
        ...
        # EPLB: maps logical experts to physical expert locations
        # Initially static: logical expert i maps to physical expert i
        self.register_buffer(
            'log2phy',
            torch.arange(num_logical_experts, dtype=torch.int32),
            persistent=False,
        )
    ...
    def refresh_placement(self, rank: int, layer_id: int, num_layers: int) -> None:
        """Refresh expert placement using EPLB based on workload history."""
        ...
        # Extract the mapping for this layer and convert to log2phy
        # log2phy shape: [num_layers, num_logical_experts, max_replicas]
        # For EPLB with no redundancy, each logical expert maps to exactly one physical expert
        new_log2phy = log2phy[0, :, 0].to(dtype=torch.int32)
        ...
        if is_main_process(rank):
            ...
            print(
                f'[rank {rank}] refreshed EPLB placement; '
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Map logical experts to physical experts using EPLB placement
        log2phy = cast(torch.Tensor, self.log2phy).to(device=flat.device)
...
class TinyMoeBlock(nn.Module):
    ...
    def refresh_placement(self, rank: int, layer_id: int, num_layers: int) -> None:
        self.moe.refresh_placement(rank, layer_id, num_layers)
...
class TinyMoeLM(nn.Module):
    ...
    def refresh_placement(self, rank: int) -> None:
        for layer_id, block in enumerate(self.blocks):
            cast(TinyMoeBlock, block).refresh_placement(rank, layer_id, self.num_layers)
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError('this demo requires CUDA because EPLB runs on CUDA tensors')
    ...
    # EPLB: no redundancy, so num_physical_experts = num_logical_experts
    ...
    base_model = unwrap_model(model)
    base_model.refresh_placement(rank)
    ...
    log(
        ...
        f'load_balancing=EPLB top_k={TOP_K}',
    )
    ...
    for step in range(args.steps):
        ...
        if step_should_profile and forward_start is not None:
            ...
            profile_sums['eplb_run'] += router_stats.get('eplb_run_ms', 0.0) / 1e3
    ...
    if profile_enabled and profile_count > 0:
        ...
        if is_main_process(rank):
            ...
            print(
                ...
                f'  refresh placement: {_fmt_idx(2)}\n'
